...
main(int argc, char **argv)
{
	...
	for (hostp = hosts; hostp->hostname != NULL; hostp++)
	{
		...
		for (resorig = res; res != NULL; res = res->ai_next, i++)
		{
			...
			if ((hostp->sockets[i] = socket(res->ai_family, res->ai_socktype, 0)) == -1) 
			{
			...
			if (fcntl(hostp->sockets[i], F_SETFL, flags | O_NONBLOCK) == -1)
				err(1, "fcntl");

			if (connect(hostp->sockets[i], (struct sockaddr *)res->ai_addr, res->ai_addrlen) == -1) 
			{
		...
		*/
		...
		for (hostp = hosts; hostp->hostname != NULL; hostp++) 
		{
			...
			for (i = 0; i < hostp->numsock; i++) 
			{
				...
				if (getsockopt(hostp->sockets[i], SOL_SOCKET, SO_ERROR, &optval, &optlen) == -1)
					err(1, "getsockopt");
